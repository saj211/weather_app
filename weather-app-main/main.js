...
function applyText() {
    showTextLetterByLetter(document.querySelector(".greeting"), texts.greeting, 50);
...
// serach based on the cities name helper func
...
//get the real weather,feels like, windspeed, precipitation,
//  humdity, max and min weather, daly forcast of the next 7 
// days, all based on the latitude and longitude 
...
// affecting the unit 
    ...
    try {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const dayName = new Date(dataStr).toLocaleDateString("en-US", {
                weekday: "short",
            });
...
async function handleDay(selectedDay) {
    ...
    const now = new Date();
    let count = 0;
// a method we can modify the number of the boxes in the hourly
    for (let j = 1; j <= 7; j++) {
        ...
        if (hourEl && weatherEl && iconEl) {
            ...
            hourEl.closest(".hours").style.display = "none";
        }
    }
    ...
    for (let i = 1; i < times.length; i++) {
        ...
        if (dayName === selectedDay) {
            ...
            if (hourEl && weatherEl && iconEl) {
                ...
                hourEl.closest(".hours").style.display = "flex";
            }
            count++;
            if (count === 7) break;
        }
...
function updateCheck() {
    ...
    const selectedSpan = document.querySelector(`#unitmenu .item[data-value="${state.unit}"] .check`);

    if (selectedSpan) {
        selectedSpan.innerHTML = '<img src="assets/images/icon-checkmark.svg" class="check-icon">';
    }
}
...
//handle the retry btn in the api err page
...
// for the day menu in hourly column
...
// for the retry btn in the api err page
retryBtn.addEventListener("click", handleRetry);

// save the user's choice as default 
